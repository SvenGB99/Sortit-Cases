        ...
        'render' => function ($node) {
            ...
            $content = Helper::renderTemplate(function () use ($node) {
                ...
                if (empty($node->props['show_headline'])) {
                    $filters[] = Helper::addFilter('woocommerce_product_upsells_products_heading', function () {}, 100);
                }
                ...
                $filters[] = Helper::addFilter('woocommerce_upsells_columns', function () use ($node) {
                    return $node->props['columns'];
                });

                $filters[] = Helper::addFilter('woocommerce_upsells_orderby', function () use ($node) {
                    return $node->props['orderby'];
                });

                $filters[] = Helper::addFilter('woocommerce_upsells_order', function () use ($node) {
                    return $node->props['order'];
                });

                $filters[] = Helper::addFilter('woocommerce_upsells_total', function () use ($node) {
                 ...
                 */
                Hook::doAction('woocommerce_after_single_product_summary', ['start' => 15, 'end' => 19]);
