    ...
    public static function config($source, $name, $relationships)
    {
        return [
            ...
            'fields' => array_filter(array_map(function ($relationship) use ($source, $name) {
                ...
                $type = Helper::getPostType($relationship['roles'][$isParent ? 'child' : 'parent']['types'][0]);
                ...
                if ($relationship['cardinality']['type'] === 'one-to-one' || ($relationship['cardinality']['type'] === 'one-to-many' && !$isParent)) {
                    ...
                    $relationshipName = Str::camelCase(['toolset', $type->name, $relationship['slug']], true);

                    $source->objectType($relationshipName, RelationshipType::config($type->name, $relationship));
